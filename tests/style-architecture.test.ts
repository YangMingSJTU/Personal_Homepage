...
describe("style architecture", () => {
  ...
  it("keeps the intro fluid canvas unobscured like the reference opening", () => {
    ...
    expect(introHero).toContain('data-particle-primitive="segmented-streamline"');
    expect(introHero).toContain('data-particle-endcap="none"');
    ...
    const drawParticleSource = particleMorph.match(/function drawParticle[\s\S]*?\n}\n\nfunction traceTaijiSeam/)?.[0] ?? "";
    expect(drawParticleSource).toContain('context.lineCap = "butt"');
    expect(drawParticleSource).not.toContain("context.arc");
