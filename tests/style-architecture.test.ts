...
describe("style architecture", () => {
  ...
  it("keeps the intro fluid canvas unobscured like the reference opening", () => {
    ...
    expect(introHero).toContain('data-fluid-transition-model="single-field-fluid-absorption"');
    expect(introHero).toContain('data-fluid-transition-flow="bounded-contraction-spiral-capture"');
    expect(introHero).toContain('data-fluid-transition-distribution="edge-55-interior-45"');
    expect(introHero).toContain('data-fluid-transition-reveal="density-sink-board-handoff"');
    expect(introHero).toContain('data-fluid-transition-capture="physical-dye-core-absorption"');
    ...
    expect(fluidVendor).toContain("float contractionRate = mix(0.72, 2.35");
    expect(fluidVendor).toContain("float angularRate = mix(0.58, 0.05");
    expect(fluidVendor).toContain("-radial * contractionRate + clockwise * angularRate");
    expect(fluidVendor).not.toContain("remainingSeconds");
    expect(fluidVendor).toContain("float steering = 1.0 - exp(-steeringRate * dt)");
    ...
    expect(fluidVendor).toContain("const force = 18 + state.random() * 14");
    ...
    expect(fluidVendor).toContain("float absorptionRate = coreCapture * mix(0.0, 14.0, captureProgress)");
    expect(fluidVendor).toContain("float desaturation = coreColorInfluence * 0.20");
    expect(fluidVendor).toContain("float toneMappedEnergy = colorEnergy / (1.0 + 0.35 * colorEnergy)");
    expect(fluidVendor).toContain("const color = generateColor();\n\tconst colorBoost");
