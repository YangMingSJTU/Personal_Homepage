...
describe("style architecture", () => {
  ...
  it("keeps the intro fluid canvas unobscured like the reference opening", () => {
    ...
    expect(introHero).toContain('data-fluid-transition-flow="terminal-spiral-avatar-capture"');
    ...
    expect(fluidVendor).toContain("mix(1.0, 0.08, terminalProgress)");
    expect(fluidVendor).toContain("mix(740.0, 2400.0");
    expect(fluidVendor).toContain("tangentialVelocity *= 1.0 - terminalProgress");
    expect(fluidVendor).toContain("coreForceGate * mix(0.92, 1.08, farZone)");
