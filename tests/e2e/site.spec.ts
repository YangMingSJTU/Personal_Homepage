...
test("expands the GitHub corner as a real page-corner curl", async ({ page }) => {
  ...
  const curl = corner.locator("[data-github-curl]");
  ...
  await expect(corner.locator("[data-github-curl-asset]")).toHaveCount(0);
  await expect(corner.locator("[data-github-corner-label]")).toHaveCount(0);
  await expect(corner.locator("[data-github-corner-underlay]")).toHaveCount(0);
  ...
  await expect(corner).toHaveAttribute("data-corner-style", "css-page-curl");
  ...
  expect(expandedBox?.width).toBeLessThanOrEqual(66);
  expect(expandedBox?.height).toBeLessThanOrEqual(66);
